...
void Interface::doseAtPoints ()
{
	...
	*input << "\n\n";
	...
	for (int j = 0; j < points.size(); j++)
	{
	    x = (*(points[j]))[0]; y = (*(points[j]))[1]; z = (*(points[j]))[2];
...
void Interface::rebinBounds()
{
	...
	if (doseList->selectedItems().contains(doseList->item(d)))
	{
	    ...
	    temp->val.resize(temp->x); temp->err.resize(temp->x);
	    for (int i = 0; i < temp->x; i++)
	    {
		temp->val[i].resize(temp->y); temp->err[i].resize(temp->y);
...
void Interface::plotAxis()
{
    ...
    if (!d || !e || !f || !g)
    {
		badInput.showMessage(tr("The coordinates of the plot must be real ") +
			     tr("numbers."));
		return;
    }
	if (j >= k)
    ...
    */
    ...
    if (file->open(QIODevice::WriteOnly | QIODevice::Text))
    {
		...
		*input << "@    xaxis  label \"" << axis << " AXIS (cm)\"" << "\n";
		*input << "@    timestamp on" << "\n";
		*input << "@    yaxis  label \"DOSE (Gy)\"" << "\n";
...
void Interface::plotLine()
{
    ...
    double dInc, xInc, yInc, zInc;
    xInc = (xf-xi)/double(res);
    yInc = (yf-yi)/double(res);
    zInc = (zf-zi)/double(res);
    dInc = sqrt(pow(xf-xi, 2) + pow(yf-yi, 2) + pow(zf-zi, 2))/double(res);
	...
	*input << "@    legend y1 0.75" << "\n";
	*input << "@    view xmin 0.250000" << "\n";
	*input << "@    xaxis  label \"R (cm)\"" << "\n";
	*input << "@    timestamp on" << "\n";
	*input << "@    yaxis  label \"DOSE (Gy)\"" << "\n";

	int n = 0; double val = 0, err = 0;
	for (int i = 0; i < data->size(); i++)
	    if (doseList->selectedItems().contains(doseList->item(i)))
	    {
		if (n)
		...
		*input << "@    s" << QString::number(n)
		...
		for (int j = 0; j < res; j++)
		{
		    ...
		    *input << "\t";
		    *input << QString::number(dInc * double(j), 'E', 4);
		    *input << "\t";
		    *input << QString::number(val, 'E', 4);
		    *input << "\t";
		    *input << QString::number(val*err, 'E', 4);
		    *input << "\n";
		}
		n++;
	    }
...
void Interface::statH(Dose* comp)
{
	...
	*input << "@    view xmin 0.250000" << "\n";
	switch (statOptBox->currentIndex())
		...
		case 0: // Sum Difference
		    *input << "@    xaxis  label \"Difference (Gy)\"" << "\n";
		    break;
		case 1: // Sum Difference / Average
		    *input << "@    xaxis  label \"Difference / Average\""
			   << "\n";
		    break;
		case 2: // Sum Difference / Max
		    *input << "@    xaxis  label \"Difference / Max\""
			   << "\n";
		    break;
		case 3: // Sum Difference / Uncertainty	
		    *input << "@    xaxis  label \"Difference / Uncertainty\""
			   << "\n";    
		    break;
		case 4: // Square of Difference
		    *input << "@    xaxis  label "
			   << "\"Square of Difference (Gy^2)\"" << "\n";
		    break;
	...
	*input << "@    timestamp on" << "\n";
	*input << "@    yaxis  label \"Normalized Frequency\"" << "\n";
...
void Interface::createLayout ()
{
    ...
    *statOptItems << "Difference"
    ...
    statFrame->setToolTip(tr("Create and display an xmgrace histogram."));
    ...
    *plotItems << "x axis" << "y axis" << "z axis" << "Free" << "Total DVH"
    ...
    plotFrame = new QGroupBox ("Create XMGrace plot");
    plotFrame->setLayout(plotLayout);
    plotFrame->setToolTip(tr("Create and display an xmgrace profile plots \n") +
