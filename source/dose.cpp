...
: QObject (0)
{
	...
	val.resize(x); err.resize(x);
	for (int i = 0; i < d.x; i++)
	{
		val[i].resize(y); err[i].resize(y);
		for (int j = 0; j < d.y; j++)
		{
			val[i][j].resize(z); err[i][j].resize(z);
			for (int k = 0; k < d.z; k++)
			{
...
double e0, double e1, double *val, double *err)
{
	...
	e0 *= b0;e1 *= b1; // Convert from fractional error to absolute error
...
double *err)
{
	...
	*val += getDose(x0,y0,z0)*(x1-xp)*(y1-yp)*(z1-zp);
	*err += pow(getDose(x0,y0,z0)*getError(x0,y0,z0)*(x1-xp)*(y1-yp)*(z1-zp),2);

	*val += getDose(x1,y0,z0)*(xp-x0)*(y1-yp)*(z1-zp);
	*err += pow(getDose(x1,y0,z0)*getError(x1,y0,z0)*(xp-x0)*(y1-yp)*(z1-zp),2);

	*val += getDose(x0,y1,z0)*(x1-xp)*(yp-y0)*(z1-zp);
	*err += pow(getDose(x0,y1,z0)*getError(x0,y1,z0)*(x1-xp)*(yp-y0)*(z1-zp),2);
	
	*val += getDose(x0,y0,z1)*(x1-xp)*(y1-yp)*(zp-z0);
	*err += pow(getDose(x0,y0,z1)*getError(x0,y0,z1)*(x1-xp)*(y1-yp)*(zp-z0),2);
	
	*val += getDose(x0,y1,z1)*(x1-xp)*(yp-y0)*(zp-z0);
	*err += pow(getDose(x0,y1,z1)*getError(x0,y1,z1)*(x1-xp)*(yp-y0)*(zp-z0),2);
	
	*val += getDose(x1,y0,z1)*(xp-x0)*(y1-yp)*(zp-z0);
	*err += pow(getDose(x1,y0,z1)*getError(x1,y0,z1)*(xp-x0)*(y1-yp)*(zp-z0),2);
	
	*val += getDose(x1,y1,z0)*(xp-x0)*(yp-y0)*(z1-zp);
	*err += pow(getDose(x1,y1,z0)*getError(x1,y1,z0)*(xp-x0)*(yp-y0)*(z1-zp),2);
	
	*val += getDose(x1,y1,z1)*(xp-x0)*(yp-y0)*(zp-z0);
	*err += pow(getDose(x1,y1,z1)*getError(x1,y1,z1)*(xp-x0)*(yp-y0)*(zp-z0),2);

	*val /= (x1-x0)*(y1-y0)*(z1-z0);
	double weight = pow((x1-xp)*(y1-yp)*(z1-zp),2);
	weight       += pow((xp-x0)*(y1-yp)*(z1-zp),2);
	weight       += pow((x1-xp)*(yp-y0)*(z1-zp),2);
	weight       += pow((x1-xp)*(y1-yp)*(zp-z0),2);
	weight       += pow((x1-xp)*(yp-y0)*(zp-z0),2);
	weight       += pow((xp-x0)*(y1-yp)*(zp-z0),2);
	weight       += pow((xp-x0)*(yp-y0)*(z1-zp),2);
	weight       += pow((xp-x0)*(yp-y0)*(zp-z0),2);
...
int Dose::strip()
{
	...
	cx.remove(x); cy.remove(y); cz.remove(z);
	cx.remove(0); cy.remove(0); cz.remove(0);
	...
	for (int i = 0; i < x; i++)
	{
		for (int j = 0; j < y; j++)
		{
			...
			val[i][j].remove(z-1); val[i][j].remove(0);
			err[i][j].remove(z-1); err[i][j].remove(0);
		}
		// Remove the first and last y value of each x line
		val[i].remove(y-1); val[i].remove(0);
		err[i].remove(y-1); err[i].remove(0);
	}
	// Remove the first and last x values
	val.remove(x-1); val.remove(0);
	err.remove(x-1); err.remove(0);
...
double xf, double yi, double yf, double zi,
double zf)
{
	*min = *max = *avg = 0;
	int flag = 0;
...
int Dose::scaleAtPoint(double px, double py, double pz, double goal, char doTri)
{
	...
	else
	{
		...
		value = getError(px, py, pz);
	}
	// After finding the dose at (px, py, pz) using interpolation, a factor is
	// caluclated so that the voxel at the position has goal dose, and each
	// other voxel is scaled appropriately
